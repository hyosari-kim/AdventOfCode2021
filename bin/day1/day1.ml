...
let _ = let result = string_of_int (snd finalState) in print_string ("part1:" ^ result)
...
let rec find_increses = function
 | [] -> 0
 | h :: t -> if (h + sum_of_til_nth 2 t) < (sum_of_til_nth 3 t) then 1 + find_increses t else find_increses t 

let _ = let result = string_of_int (find_increses data) in print_string ("part2:" ^ result)